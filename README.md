...
#### 2. Live API & Web Scraping Mode
Pulls real listening data from Last.fm and scrapes artist hero imagery using credentials from your `.env`:
```bash
...
| `--live` | | Run live Last.fm queries and web scraping | |
...
2. Go to the **Run & Debug** tab (`Ctrl+Shift+D` / `Cmd+Shift+D`) and select a launch profile:
   ...
   - `🌐 Debug: Live Artist Collage (.env)`: Live Last.fm API query and web scraping.
