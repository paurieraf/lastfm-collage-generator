...
### 1.7 Image Fetching and Web Retrieval Mechanism
...
- **Artist Images** (`collage.py:228-261`):
  - Last.fm API does not provide artist images, so retrieval is used.
...
4. **Web Retrieval Fragility**:
   - Observation 1.7 shows `ArtistCollageBuilder._get_artist_image` relies on retrieval `https://www.last.fm/music/<artist>` and locating the class `.header-new-background-image`.
   - Last.fm does not provide artist images via API. Retrieval lacks custom User-Agent headers, session reuse, timeout limits, and fallback strategies if Last.fm updates DOM structure, returns Cloudflare challenges, or rate-limits/blocks requests.
...
6. **Missing Tests and Verification Void**:
   ...
   - Therefore, any future modifications or refactoring lack regression verification unless a test suite with mocks (for Last.fm API and web retrieval) is introduced.
...
| **Artist Image Pipeline** | `ArtistCollageBuilder` (`collage.py`) | Web fetch `last.fm/music/<artist>` via `BeautifulSoup` & `html5lib` | Extracts `.header-new-background-image`, thumbnails to 300x300. Falls back to blank tile on 404/not found. |
